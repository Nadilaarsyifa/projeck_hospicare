...
if (!empty($_POST['edit_semivip_validate'])) {
    $statusupload = 1;
    $target_dir = "../assets/img/";

    if ($statusupload == 1) {
        // Memperbaiki query update
        $query = mysqli_query($conn, "UPDATE tb_kamarsemivip SET  terisi='$terisi', status_kamar='$status' WHERE id='$id'");
        if ($query) {
            $message = '<script>alert("Data berhasil diupdate"); window.location="../kamarsemivip";</script>';
        } else {
            $message = '<script>alert("Data gagal diupdate"); window.location="../kamarsemivip";</script>';
        }
    }
}
echo $message;
