...
if (!empty($_POST['edit_vip_validate'])) {
    $statusupload = 1;
    $target_dir = "../assets/img/";

    if ($statusupload == 1) {
        // Memperbaiki query update
        $query = mysqli_query($conn, "UPDATE tb_kamarvip SET  terisi='$terisi', status_kamar='$status' WHERE id='$id'");
        if ($query) {
            $message = '<script>alert("Data berhasil diupdate"); window.location="../kamarvip";</script>';
        } else {
            $message = '<script>alert("Data gagal diupdate"); window.location="../kamarvip";</script>';
        }
    }
}
echo $message;
