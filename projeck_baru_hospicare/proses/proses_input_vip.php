...
if (!empty($_POST['input_kamar_validate'])) {
    ...
    if ($statusupload == 1) {
        $select = mysqli_query($conn, "SELECT * FROM tb_kamarvip WHERE no_kamar = '$no_kamar'");
        if (mysqli_num_rows($select) > 0) {
            $message = '<script>alert("no kamar yang dimasukkan telah ada"); window.location="../kamarvip";</script>';
        } else {
            $query = mysqli_query($conn, "INSERT INTO tb_kamarvip (foto, no_kamar, nama_kamar, kapasitas,terisi, status_kamar, fasilitas) VALUES ('$foto_name','$no_kamar','$nama_kamar','$kapasitas', '$terisi', '$status', '$fasilitas')");
