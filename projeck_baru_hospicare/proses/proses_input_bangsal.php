...
if (!empty($_POST['input_kamar_validate'])) {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == UPLOAD_ERR_OK) {
        ...
        if ($cek === false) {
            $message =  '<script>alert("Ini bukan file gambar"); window.location="../bangsal";</script>';
            $statusupload = 0;
        } else {
            if ($_FILES['foto']['size'] > 50000000) { // 50MB
                $message = '<script>alert("File foto yang diupload terlalu besar"); window.location="../bangsal";</script>';
                $statusupload = 0;
            } else {
                if ($file_extension != "jpeg" && $file_extension != "png" && $file_extension != "jpg" && $file_extension != "gif") {
                    $message = '<script>alert("Maaf, hanya diperbolehkan gambar yang memiliki format jpeg, png, jpg, dan gif"); window.location="../bangsal";</script>';
                    $statusupload = 0;
                }
            }
        }
        ...
        if ($statusupload == 1) {
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
                ...
                $message = '<script>alert("Maaf terjadi kesalahan file tidak dapat di upload"); window.location="../bangsal";</script>';
                $statusupload = 0;
            }
        }
    }
    ...
    if ($statusupload == 1) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if ($query) {
                ...
                $message = '<script>alert("Data gagal dimasukkan"); window.location="../bangsal";</script>';
            }
        }
    }
}
